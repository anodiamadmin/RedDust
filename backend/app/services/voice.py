...
from app.schemas import ChatResponse, VoiceResponse
...
async def call_gemini_voice(audio_bytes, mime_type, session_id) -> VoiceResponse:
    ...
    contents.append(
        types.Content(
            ...
            parts=[
                ...
                types.Part(
                    text=(
                        "The audio above is the user's voice message. "
                        "Transcribe exactly what they said, then respond as RedDust. "
                        "Return JSON with keys: transcription, reply, followup_question."
                    )
                ),
            ],
        )
    ...
    # Call Gemini (identical to llm.py)
    ...
    # Parse (identical to llm.py)
    try:
        data = _parse_response(response.text)
    except ResponseParseError:
        data = {"transcription": "", "reply": response.text[:1000], "followup_question": None}

    # Persist to session store using transcription as the user turn
    user_text = data.get("transcription", "")
    await _append_message_async(session_id, "user", user_text)
