-- ============================================================================
-- RedDust / Syan PostgreSQL Schema  —  VERSION 2
-- Aligned with ER Diagram (Updated): Database_schema_V2.png
--
-- Changes from V1 → V2 are marked with: -- [CHANGE vX]: description
--
...
-- ============================================================================
-- [CHANGE v2]: NEW ENUM TYPES
--   The diagram explicitly shows enums for speaker, source, and visibility.
--   Using native PostgreSQL ENUM improves storage efficiency and enforces
--   values at the type level rather than inline CHECK constraints.
-- ============================================================================

-- Replaces the CHECK (speaker IN ('user','syan','system','tool')) in conversation_turn.
-- Diagram shows enum values: user | ai | system
-- Note: 'syan' is the AI speaker name; mapped to 'ai' in the enum for generality.
-- [CHANGE v2]: speaker stored as enum type, not raw text with CHECK
DO $$ BEGIN
    CREATE TYPE speaker_enum AS ENUM ('user', 'ai', 'system', 'tool');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Replaces free-text source_type in wellbeing_signal.
-- Diagram shows enum values: app | wearable | manual | imported | inferred
-- [CHANGE v2]: source_type stored as enum type, not free-text
DO $$ BEGIN
    CREATE TYPE source_enum AS ENUM ('app', 'wearable', 'manual', 'imported', 'inferred');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

-- Replaces CHECK (visibility_state IN ('active','hidden','passive')) in user_dimension_state.
-- Diagram adds 'archived' and 'deleted' states.
-- [CHANGE v2]: visibility_state enum expanded; 'passive' → 'archived'/'deleted' added
DO $$ BEGIN
    CREATE TYPE visibility_enum AS ENUM ('active', 'archived', 'hidden', 'deleted');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- [CHANGE v2]: Unique constraint on lower(email) unchanged — confirmed in diagram
...
CREATE TABLE IF NOT EXISTS conversation_turn (
    ...
    -- [CHANGE v2]: speaker column type changed from TEXT+CHECK to speaker_enum
    --   Old: speaker TEXT NOT NULL CHECK (speaker IN ('user','syan','system','tool'))
    --   New: speaker speaker_enum NOT NULL
    --   Diagram explicitly shows speaker_enum with values: user | ai | system
...
CREATE TABLE IF NOT EXISTS wellbeing_signal (
    ...
    -- [CHANGE v2]: source_type changed from free TEXT to source_enum
    --   Old: source_type TEXT NOT NULL
    --   New: source_type source_enum NOT NULL
    --   Diagram shows source_enum: app | wearable | manual | imported | inferred
...
CREATE TABLE IF NOT EXISTS user_dimension_state (
    ...
    -- [CHANGE v2]: visibility_state changed from TEXT+CHECK to visibility_enum
    --   Old: visibility_state TEXT NOT NULL CHECK (visibility_state IN ('active','hidden','passive'))
    --   New: visibility_state visibility_enum NOT NULL
    --   Diagram adds 'archived' and 'deleted'; removes 'passive'
    visibility_state  visibility_enum NOT NULL,
    ...
    -- [CHANGE v2]: CHECK updated — 'active' still requires dashboard_rank;
    --   non-active states (archived/hidden/deleted) must have NULL rank
...
CREATE TABLE IF NOT EXISTS user_dimension_state_event (
    ...
    user_id           UUID NOT NULL REFERENCES app_user(user_id) ON DELETE CASCADE,
    dimension_id      SMALLINT NOT NULL REFERENCES wellbeing_dimension(dimension_id),
    -- [CHANGE v2]: old_state / new_state changed from TEXT+CHECK to visibility_enum
    old_state         visibility_enum,
    new_state         visibility_enum NOT NULL,
    reason_code       TEXT,
    reason_text       TEXT,
...
CREATE TABLE IF NOT EXISTS conversation_dimension_score (
    ...
    -- [CHANGE v2]: invalidated_at added
    --   Allows soft-invalidation of stale scores without hard deletion.
    --   NULL = valid; non-NULL = invalidated at that timestamp.
...
CREATE TABLE IF NOT EXISTS session_dimension_score (
    ...
    algorithm_version TEXT NOT NULL DEFAULT '1.0',
    calculated_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
    -- [CHANGE v2]: invalidated_at added (mirrors conversation_dimension_score)
    invalidated_at    TIMESTAMPTZ,
...
CREATE TABLE IF NOT EXISTS user_soul_score_snapshot (
    ...
    calculated_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
    -- [CHANGE v2]: invalidated_at added
    --   Diagram notes "score snapshot tables include calculated_at and invalidated_at"
    invalidated_at        TIMESTAMPTZ,
...
-- ============================================================================
-- [CHANGE v2]: NEW TABLE — audio_features
--   The diagram shows a new "audio_features" entity linked 1:1 to music_track.
--   Stores acoustic/psychoacoustic properties used by the recommendation engine
--   to match music to wellbeing dimensions (e.g. high valence + high energy for
--   motivation, low tempo + low loudness for sleep readiness).
--
--   All numeric features use NUMERIC(4,3) matching diagram annotation.
--   feature_source TEXT: records where features came from
--       (e.g. 'spotify_api', 'essentia', 'manual', 'ml_model').
--   extracted_at: when the features were last computed/fetched.
--   One row per track enforced by PRIMARY KEY = track_id (1:1 with music_track).
-- ============================================================================

CREATE TABLE IF NOT EXISTS audio_features (
    -- PK is also FK → music_track; enforces exactly one features row per track
    track_id           UUID PRIMARY KEY REFERENCES music_track(track_id) ON DELETE CASCADE,

    -- Rhythmic / energy features
    tempo_bpm          NUMERIC(8,2),                            -- beats per minute; wider range needed
    time_signature     NUMERIC(4,3),                            -- e.g. 4/4 = 4.000
    energy             NUMERIC(4,3) CHECK (energy BETWEEN 0 AND 1),
    loudness_db        NUMERIC(5,2),                            -- typically -60 to 0 dB

    -- Emotional / tonal features
    valence            NUMERIC(4,3) CHECK (valence BETWEEN 0 AND 1),    -- 0=negative, 1=positive
    arousal            NUMERIC(4,3) CHECK (arousal BETWEEN 0 AND 1),    -- 0=calm, 1=excited
    ...
    -- Tonal features
    key_note           SMALLINT CHECK (key_note BETWEEN 0 AND 11),      -- 0=C, 1=C#, …, 11=B
    mode               SMALLINT CHECK (mode IN (0, 1)),                 -- 0=minor, 1=major

    -- Wellbeing suitability scores (computed by ML layer, stored for fast lookup)
    sleep_suitability      NUMERIC(4,3) CHECK (sleep_suitability BETWEEN 0 AND 1),
    motivation_suitability NUMERIC(4,3) CHECK (motivation_suitability BETWEEN 0 AND 1),
    focus_suitability      NUMERIC(4,3) CHECK (focus_suitability BETWEEN 0 AND 1),
    confidence             NUMERIC(4,3) CHECK (confidence BETWEEN 0 AND 1),  -- feature extraction confidence

    -- Provenance
    feature_source     TEXT,          -- e.g. 'spotify_api', 'essentia', 'ml_model', 'manual'
    extracted_at       TIMESTAMPTZ    -- when features were last computed / fetched
);
...
CREATE TABLE IF NOT EXISTS music_recommendation_reaction (
    ...
    -- [CHANGE v2]: All boolean columns explicitly NOT NULL DEFAULT FALSE
    --   Old: played BOOLEAN NOT NULL DEFAULT FALSE (was correct), but skipped/replayed/
    --        played_along were inconsistently defined. Diagram note says:
    --        "Reaction booleans are NOT NULL DEFAULT false" — enforced uniformly here.
    ...
    free_text         TEXT,
    -- [CHANGE v2]: reacted_at changed from TIMESTAMPTZ NOT NULL DEFAULT now()
    --   Diagram annotates this column as "NOT NULL DEFAULT now()" explicitly.
    --   Was already correct in V1 but diagram calls it out as a business rule.
...
-- ============================================================================
-- 7. INITIALISE ALL DIMENSIONS FOR A NEW USER
-- ============================================================================
...
BEGIN
    INSERT INTO user_dimension_state (
        user_id,
        dimension_id,
        -- [CHANGE v2]: cast to visibility_enum
        visibility_state,
        dashboard_rank,
        priority_score,
        activation_reason,
    ...
    SELECT
        NEW.user_id,
        d.dimension_id,
        -- [CHANGE v2]: literal cast to visibility_enum type
        CASE WHEN d.is_primary THEN 'active'::visibility_enum ELSE 'hidden'::visibility_enum END,
...
BEGIN
    ...
    WHERE c.session_id = p_session_id
      -- [CHANGE v2]: exclude invalidated scores from aggregation
      AND cds.invalidated_at IS NULL
...
base AS (
    ...
    LEFT JOIN session_dimension_score sds
     ...
     -- [CHANGE v2]: exclude invalidated session scores
     AND sds.invalidated_at IS NULL
    WHERE d.is_enabled = TRUE
),
history AS (
    SELECT
        ...
        (
            ...
            WHERE us.user_id = p_user_id
              ...
              -- [CHANGE v2]: exclude invalidated scores from history
              AND hs.invalidated_at IS NULL
...
BEGIN
    ...
    DO UPDATE SET
        ...
        calculated_at = now();
        -- Note: invalidated_at is NOT reset on upsert; only set explicitly when invalidating.
END;
$$;

-- Latest score for every user/dimension — [CHANGE v2]: filters out invalidated snapshots
...
-- [CHANGE v2]: only include non-invalidated snapshots in the live view
WHERE s.invalidated_at IS NULL
ORDER BY s.user_id, s.dimension_id, s.calculated_at DESC;

-- Dashboard: active dimensions only — unchanged logic, benefits from view filter above
...
INSERT INTO soul_score_config (
    config_id, methodology_version, score_min, score_max,
    short_term_days, long_term_days, default_dashboard_dimensions
)
...
INSERT INTO soul_score_duration_policy (
    duration_code, sort_order, current_weight, short_history_weight, long_history_weight, description
)
VALUES
...
INSERT INTO wellbeing_dimension (
    dimension_id, dimension_name, negative_dimension,
    duration_code, is_primary, default_dashboard_rank, research_basis
)
VALUES
...
INSERT INTO user_dimension_state (
    user_id, dimension_id, visibility_state,
    dashboard_rank, priority_score, activation_reason, activated_at
)
SELECT
    u.user_id,
    d.dimension_id,
...
-- ============================================================================
-- EXAMPLE EXECUTION AT THE END OF A SESSION (unchanged)
-- ============================================================================
--
...
--
-- To invalidate a score without deleting it:
--      UPDATE reddust.user_soul_score_snapshot
--      SET invalidated_at = now()
--      WHERE user_id = '<user-uuid>' AND dimension_id = <dim_id>;
--
-- ============================================================================
